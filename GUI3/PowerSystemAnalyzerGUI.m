classdef PowerSystemAnalyzerGUI < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function RunAnalysisButtonPushed(app, event)
            ...
            try
                ...
                app.ExporttoExcelButton.Enable = 'on'; % Enable export button
        ...
        function ExporttoExcelButtonPushed(app, event)
            try
                app.StatusField.Value = 'Exporting to Excel...';
                data_to_export = app.LoadFlowUITable.Data;
                
                [~, name, ~] = fileparts(app.FileNameField.Value);
                outFileName = ['FDLF_Results_' name '.xlsx'];
                
                writetable(data_to_export, outFileName, 'Sheet', 'LoadFlowResults');
                
                app.StatusField.Value = ['Results successfully saved to ' outFileName];
                uialert(app.UIFigure, ['Results successfully saved to ' outFileName], 'Export Successful');
            catch ME
                app.StatusField.Value = ['ERROR exporting file: ' ME.message];
                uialert(app.UIFigure, ['Could not save the file: ' ME.message], 'Export Error');
            end
        end
    end
    ...
    methods (Access = public)
        ...
        function createComponents(app)
            ...
            app.UIFigure.Position = [100 100 1150 650];
            ...
            app.LoadFlowResultsTabGroup.Position = [20 20 800 500];
            ...
            app.LoadFlowUITable.ColumnName = {'Bus'; 'Type'; 'V (p.u.)'; 'Angle (deg)'; 'P Gen (MW)'; 'Q Gen (MVAr)'; 'P Load (MW)'; 'Q Load (MVAr)'};
            app.LoadFlowUITable.RowName = {};
            app.LoadFlowUITable.Position = [15 100 770 400];

            % Create LoadFlowSummaryLabel
            app.LoadFlowSummaryLabel = uilabel(app.LoadFlowResultsTab);
            app.LoadFlowSummaryLabel.FontSize = 14;
            app.LoadFlowSummaryLabel.Position = [15 510 770 50];
            app.LoadFlowSummaryLabel.Text = 'Load Flow Summary';

            % Create ExporttoExcelButton
            app.ExporttoExcelButton = uibutton(app.LoadFlowResultsTab, 'push');
            ...
            app.ExporttoExcelButton.FontSize = 14;
            app.ExporttoExcelButton.Position = [300 40 200 35];
    ...
    methods (Access = private)
        ...
        function displayLoadFlowResults(app, V, Vang_deg_final, PG_final_MW, QG_final_MVAr, total_loss_MW, iter, solveTime)
            ...
            PL_MW = app.busData(:,5); % Get load data directly from original busData
            QL_MVAr = app.busData(:,6);
            Vmag = abs(V);
            
            T = table(busNo, busType, Vmag, Vang_deg_final, PG_final_MW, QG_final_MVAr, PL_MW, QL_MVAr);
            T.Properties.VariableNames = {'Bus', 'Type', 'V_pu', 'Angle_deg', 'P_Gen_MW', 'Q_Gen_MVAr', 'P_Load_MW', 'Q_Load_MVAr'};
